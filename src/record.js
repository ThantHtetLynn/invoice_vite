...
export const createRecordRow = ({ id, name, price }, quantity) => {
  ...
  const recordQuantity = recordRow.querySelector(".record-product-quantity");
...
export const removeRecord = (rowId) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      //   text: "The product you selected has been removed.",
      //   icon: "success"
      // });
    }
  });
...
export const quantityAdd = (rowId) => {
  ...
  const recordQuantity = currentRecordRow.querySelector(
...
export const recordGroupHandler = (event) => {
  if (event.target.classList.contains("record-remove")) {
    ...
    // removeRecord();
  } else if (event.target.classList.contains("quantity-add-btn")) {
    // console.log("U added:");
    const currentRecordRow = event.target.closest(".record-row");
    updateRecordQuantity(currentRecordRow.getAttribute("row-id"), 1);
  } else if (event.target.classList.contains("quantity-sub-btn")) {
    // console.log("U added:");
    const currentRecordRow = event.target.closest(".record-row");
    updateRecordQuantity(currentRecordRow.getAttribute("row-id"), -1);
  }
